...
>对于#include<头文件>，表示是系统文件，编译**会先从**标准库路径下搜索，编译器设置的头文件路径-->系统变量
>
...
> 1. **`#`（字符串化操作符）**：
>
>    - `#`用于将宏参数转换为字符串。在宏定义中，如果在宏体中使用`#`，它会将后面的宏参数转换成一个字符串。这被称为**字符串化**操作符。
>
>    - 例如：
>
>      ```c
...
>      ```
>
>    - 在这个例子中，`STR(Hello)`将宏参数`Hello`转换为了字符串。
>
> 2. **`##`（连接操作符）**：
>
>    - `##`用于将两个宏参数拼接在一起。在宏定义中，使用`##`可以将两个标识符或宏参数连接成一个新的标识符或表达式。
>
>    - 例如：
>
>      ```c
...
>      ```
>
>    - 在这个例子中，`CONCAT(x, y)`会将`x`和`y`拼接成`xy`，从而访问变量`xy`。
>
> **总结：**
>
> - `#`将宏参数转换为字符串。
> - `##`将两个宏参数拼接在一起形成一个新的标识符或表达式。
