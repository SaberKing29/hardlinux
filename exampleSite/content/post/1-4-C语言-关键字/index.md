...
>无条件跳转
>
>`goto` 是 C 语言中的一种无条件跳转语句。使用 `goto` 语句可以将程序直接跳转到标号（label）所在的位置，继续执行代码。虽然这种跳转方式可以在某些场景下简化代码，但是通常建议避免使用 `goto`，因为它可能会导致代码可读性差、逻辑复杂。
>
>**语法：**
>
>```c
...
>        i++;
>        if (i == 3) {
>            goto skip; // 当 i 等于 3 时跳转到 skip 位置
>        }
>        printf("i = %d\n", i);
>    }
>
>skip:
>    printf("Skipped when i = 3\n");
>    return 0;
>}
>```
>
>**使用场景：**
>
>- 在复杂的嵌套循环或条件分支中，当需要中断或跳出多层嵌套时，`goto` 可以简化逻辑。
>- 错误处理：在程序的多个地方可能发生错误的情况下，`goto` 有时可以用于统一的错误处理代码。
>
>**注意事项：**
>
>- `goto` 的过度使用会导致“面条式代码”（spaghetti code），难以维护和调试。
>- 应尽量使用结构化编程（如循环、条件分支等）替代 `goto`，只有在非常必要的情况下才使用它。
...
>   **使用场景：**
>
>   - **硬件寄存器**：访问与硬件设备相关的寄存器时，寄存器的值可能会在不通知程序的情况下发生变化，`volatile` 可以确保程序每次访问时都读取最新的值。
>   - **中断处理程序中的变量**：在中断处理程序中，变量的值可能会被中断改变，`volatile` 可以确保主程序访问这些变量时获取到最新的值。
>   - **多线程环境中的共享变量**：多个线程共享的变量可能会被一个线程修改，其他线程读取时需要获取最新的值，使用 `volatile` 可以保证正确读取。
>
>   **示例：**
>
>   ```c
...
>   ```
>
>   在上面的代码中，`flag` 是 `volatile` 的，因为它的值可能会被其他线程或中断服务程序改变。如果没有使用 `volatile` 修饰，编译器可能会对 `flag == 0` 进行优化，将其值缓存到寄存器中，导致 `someFunction` 函数永远检测不到 `flag` 的变化。
>
>   **注意事项：**
>
>   1. **仅防止优化**：`volatile` 仅用于防止编译器优化，不提供原子性操作或线程同步的保证。如果多个线程同时修改 `volatile` 变量，仍然可能会出现数据竞争问题，建议结合 `volatile` 和同步机制（如 `mutex` 或 `atomic` 操作）一起使用。
>
>   2. **与 `const` 一起使用**：`volatile` 和 `const` 可以一起使用。`volatile` 表示变量的值可以改变，`const` 表示变量不可被程序修改。例如，一个只读的硬件寄存器，可以用 `volatile const` 来修饰：
>
>      ```c
>      volatile const int *p = (int *)0x4000;  // 硬件寄存器地址
>      ```
